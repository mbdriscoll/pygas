...
// Returns a uniformly distributed double between 0.5 and 1.5
double randd(void) {
    return (double) 0.5 + (rand() / (double) RAND_MAX);
}
...
int main(int argc, char *argv[]) {
    ...
    A_bytes = B_bytes = C_bytes = nn_per_rank*sizeof(double);
    double *A_ij = (double *) malloc(A_bytes);
    double *B_ij = (double *) malloc(B_bytes);
    double *C_ij = (double *) malloc(C_bytes);
    ...
    MPI_Sendrecv_replace(A_ij, nn_per_rank, MPI_DOUBLE,
            row_shift_dest, 0, row_shift_src, 0, row, &row_stat);
    MPI_Sendrecv_replace(B_ij, nn_per_rank, MPI_DOUBLE,
    ...
    for (int k = 0; k < sqrt_P; k++) {
        ...
        MPI_Sendrecv_replace(&A_ij[0], nn_per_rank, MPI_DOUBLE,
                row_dest, 0, row_src, 0, row, &row_stat);
        MPI_Sendrecv_replace(&B_ij[0], nn_per_rank, MPI_DOUBLE,
    ...
    // Print performance
    if (rank == 0) {
        double time = (double) (t_end.tv_sec  - t_start.tv_sec) +
            1e-6 * (double) (t_end.tv_usec - t_start.tv_usec);
        double gflops = 1e-9 * pow(n, 3) / time;
